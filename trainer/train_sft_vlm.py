...
if __name__ == "__main__":
    ...
    parser.add_argument("--save_weight", default="sft_vlm", type=str, help="保存的权重文件名")
    parser.add_argument("--epochs", type=int, default=3, help="训练的轮数")
    parser.add_argument("--batch_size", type=int, default=4, help="每批次的样本数量")
    ...
    parser.add_argument("--data_path", type=str, default="../dataset/sft_data.jsonl", help="预训练数据的路径")
    parser.add_argument("--images_path", type=str, default="../dataset/sft_images", help="预训练图像的路径")
    parser.add_argument("--use_wandb", action="store_true", help="是否使用wandb进行日志记录")
    parser.add_argument("--wandb_project", type=str, default="VLM-SFT", help="wandb项目名称")

    # resume training args
    parser.add_argument('--from_weight', default='pretrain_vlm', type=str, help="加载的权重文件")
    ...
    # NOTE: here, we refreeze LLM weights during SFT
    parser.add_argument('--freeze_llm', default=0, type=int, choices=[0, 1], help="是否冻结LLM权重（0表示否，1表示是）")
    ...
    if args.use_wandb and is_main_process():
        ...
        wandb_run_name = f"miniVLM-SFT-Epoch-{args.epochs}-BatchSize-{args.batch_size}-LearningRate-{args.learning_rate}"
    ...
    """
        sft_training vs pretraining区别:
        1. 数据集不同: pretrain的数据是大规模的描述性图片对, 格式简单。而sft的数据是高质量的指令-应答对, 指令多样化（描述、问答、推理、比较等）。
        2. 核心目标不同: pretrain的目标是建立视觉与语言的基础联系, 强调通用性和广泛适用性。sft的目标是提升模型在特定任务上的表现, 激活模型的对话与推理能力。
        3. 训练策略不同: pretrain通常使用较大的batch size和更长的训练时间, 以捕捉丰富的视觉语言特征。sft则更注重微调和优化, 可能采用较小的batch size和更精细的学习率调整。
    """
