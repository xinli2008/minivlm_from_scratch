...
from trainer.train_pretrain_vlm import setup_seed
...
def init_model(args):
    ...
    if 'model' in args.load_from:
        ...
        missing_keys, unexpected_keys = model.load_state_dict({k: v for k, v in state_dict.items() if 'mask' not in k}, strict=False)
        print("=> succeed to load pretrained VLM weights with {} missing keys and {} unexpected keys.".format(len(missing_keys), len(unexpected_keys)))
    else:
        model = AutoModelForCausalLM.from_pretrained(args.load_from, trust_remote_code=True)
        model.vision_encoder, model.processor = VLMModel.get_vision_model("./model/vision_model/clip-vit-base-patch16")
    
    print(f'=> The total parameter of VLM is {sum(p.numel() for p in model.parameters() if p.requires_grad) / 1e6:.2f} M(illion)')
...
def main():
    ...
    parser.add_argument('--save_dir', default='output', type=str, help="模型权重目录")
    parser.add_argument('--weight', default='pretrain_vlm', type=str, help="权重名称前缀（pretrain_vlm, sft_vlm）")
    ...
    for image_file in sorted(os.listdir(args.image_dir)):
        if image_file.lower().endswith(('.png', '.jpg', '.jpeg', '.bmp')):
            # Fix Seed, or setup_seed(random.randint(1, 10000))
            setup_seed(2026) 
            image_path = os.path.join(args.image_dir, image_file)
            image = Image.open(image_path).convert('RGB')
            pixel_values = model.image2tensor(image, preprocess).to(args.device).unsqueeze(0)
